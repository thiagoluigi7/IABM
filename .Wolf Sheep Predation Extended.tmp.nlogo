globals [ max-sheep current-sheep current-wolf ]  ; don't let the sheep population grow too large
...
to setup
  ...
  [
    set max-sheep 10000
    set current-sheep 10000
  ]
  [
    set max-sheep 30000
    set current-sheep 30000
  ]
...
to go
  ...
  ask sheep [
    ...
    if model-version = "sheep-wolves-grass" [
      ...
      death-sheep ; sheep die from starvation only if running the sheep-wolves-grass model version
    ]
  ...
  ask wolves [
    ...
    death-wolf ; wolves die if they run out of energy
...
to reproduce-sheep  ; sheep procedure
  if random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
    ...
    Set current-sheep current-sheep + 1
  ]
end

to reproduce-wolves  ; wolf procedure
  if current-sheep > (current-wolf * 2)
  [
    if random-float 100 < wolf-reproduce
    [  ; throw "dice" to see if you will reproduce
      set energy (energy / 2)               ; divide energy between parent and offspring
      hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
      Set current-wolf current-wolf + 1
    ]
  ]
...
to death-wolf  ; turtle procedure (i.e. both wolf and sheep procedure)
